...
file_handler = RotatingFileHandler(
    LOG_DIR / "paysentinel.log", maxBytes=10 * 1024 * 1024, backupCount=5
)
...
sec_handler = RotatingFileHandler(
    LOG_DIR / "security.log", maxBytes=5 * 1024 * 1024, backupCount=3
)
...
def sanitize_language(lang: str) -> str:
    return lang if lang in {'English', 'Kannada', 'Hindi', 'Tamil', 'Telugu'} else 'English'
...
def _require_api_key(f):
    ...
    In production, missing key configuration is treated as a server misconfiguration.
    """
    @wraps(f)
    def wrapped(*args, **kwargs):
        ...
        if not required:
            ...
            strict = os.environ.get("PAYSENTINEL_STRICT_SECURITY", "").lower() == "true"
            if (env == "production" or strict) and not app.testing:
                logger.critical("Protected endpoint called without PAYSENTINEL_API_KEY in production")
...
def _cache_signing_key() -> str | None:
    """Return cache signing key if configured."""
    key = os.environ.get("PAYSENTINEL_MODEL_CACHE_KEY", "").strip()
    return key or None


def _cache_signature(cache_file: Path, cache_meta: Path, key: str) -> str:
    """Build HMAC signature for cache artifacts."""
    digest = hmac.new(key.encode(), digestmod=hashlib.sha256)
    digest.update(cache_file.read_bytes())
    digest.update(cache_meta.read_bytes())
...
def _get_cached_detector(merchant_name: str, contamination: float) -> PaySentinelDetector | None:
    ...
    cache_sig = MODEL_CACHE_DIR / f"detector_{cache_key}.sig"

    if not cache_file.exists() or not cache_meta.exists() or not cache_sig.exists():
        return None
    ...
    try:
        ...
        signing_key = _cache_signing_key()
        if not signing_key and not app.testing:
            logger.warning("Model cache signature key missing; skipping cache load for safety")
            return None
        if signing_key:
            expected = cache_sig.read_text().strip()
            actual = _cache_signature(cache_file, cache_meta, signing_key)
            if not hmac.compare_digest(expected, actual):
                logger.warning("Detector cache signature mismatch; refusing to load cached model")
                return None
...
def _save_detector_cache(detector: PaySentinelDetector, merchant_name: str, contamination: float) -> None:
    ...
    try:
        ...
        signing_key = _cache_signing_key()
        if signing_key:
            cache_sig.write_text(_cache_signature(cache_file, cache_meta, signing_key))
        elif cache_sig.exists():
...
def health():
    ...
    # Detailed diagnostics require explicit internal check + valid API key.
    if request.headers.get('X-Internal-Check') == 'true':
        required_key = os.environ.get("PAYSENTINEL_API_KEY")
        provided_key = request.headers.get("X-API-Key")
        if app.testing or (required_key and provided_key == required_key):
            status.update({
            ...
            logger.warning("Rejected internal health diagnostics request without valid API key")
      ...
      - in: body
        ...
        schema:
          ...
          properties:
            ...
            language: {type: string, enum: [English, Kannada, Hindi, Tamil, Telugu], default: English}
...
def stream():
    ...
    allowed_origin = os.environ.get("PAYSENTINEL_ALLOWED_ORIGIN", "").strip()
    stream_headers = {
    ...
    if allowed_origin:
        stream_headers['Access-Control-Allow-Origin'] = allowed_origin
...
def explain_fraud():
    ...
    try:
        ...
        if language not in ('English', 'Kannada', 'Hindi', 'Tamil', 'Telugu'): language = 'English'
        ...
        explanations = {
            ...
            'Tamil': (
                f"₹{amount:,.0f} பரிவர்த்தனைக்கு {score:.0f}/100 அபாய மதிப்பெண் உள்ளது. "
                f"{'உடனே உங்கள் வங்கியை தொடர்பு கொள்ளுங்கள்.' if score > 75 else 'இந்த பரிவர்த்தனையை கவனத்தில் வைத்துக் கொள்ளுங்கள்.'}"
            ),
            'Telugu': (
                f"₹{amount:,.0f} లావాదేవీకి {score:.0f}/100 ప్రమాద స్కోర్ ఉంది. "
                f"{'వెంటనే మీ బ్యాంకును సంప్రదించండి.' if score > 75 else 'ఈ లావాదేవీని గమనిస్తూ ఉండండి.'}"
            )
        }
