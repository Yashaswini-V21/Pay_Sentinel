...
def make_pdf(merchant: str, results: pd.DataFrame, fp: dict) -> bytes:
    ...
    if fraud_count == 0:
        ...
        advisory_title = "ಕನ್ನಡ ಸಲಹೆ (KANNADA ADVISORY)"
    elif fraud_count <= 2:
        kannada_msg = "ಕೆಲವು ಸಂಶಯಾಸ್ಪದ ವ್ಯವಹಾರಗಳು ಪತ್ತೆಯಾಗಿವೆ. ದಯವಿಟ್ಟು ಪರಿಶೀಲಿಸಿ."
        advisory_title = "ಕನ್ನಡ ಸಲಹೆ (KANNADA ADVISORY)"
    else:
    ...
    if not unicode_font_available:
        if fraud_count == 0:
            kannada_msg = "Your store appears safe. No suspicious transactions were detected."
        elif fraud_count <= 2:
            kannada_msg = "A few suspicious transactions were detected. Please review promptly."
        else:
            kannada_msg = "Warning: Multiple suspicious transactions detected. Contact bank immediately."
        advisory_title = "Regional Advisory"

    pdf.set_fill_color(255, 248, 220)
    pdf.set_xy(10, pdf.get_y())
    pdf.set_font(default_font, "B", 10)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(190, 8, advisory_title, ln=True, align="L", fill=True)
