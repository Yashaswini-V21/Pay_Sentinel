...
def engineer(df):
    ...
    # Sender trust signal based on historical familiarity in this stream.
    sender_seen_before = df.groupby('sender').cumcount()
    tx_index = np.arange(len(df)) + 1
    sender_occurrences = sender_seen_before + 1
    sender_frequency = sender_occurrences / tx_index
    df['sender_graph_weight'] = sender_frequency.clip(0.05, 0.95)
    ...
    # Deterministic risk proxy for cross-merchant abuse and synthetic identity behavior.
    df['sender_cross_merchant_risk'] = (
        (sender_seen_before == 0).astype(float) * 0.45
        + ((df['hour'] < 6) | (df['hour'] > 22)).astype(float) * 0.30
        + (df['amt_ratio_median'] > 3).astype(float) * 0.25
        + (df['vel_15m'] > 4).astype(float) * 0.20
    ).clip(0, 1)
...
class PaySentinelDetector:
    ...
    def fit(self, df):
        ...
        self.iforest.fit(Xs)
        self.svm.fit(Xs)
        # LOF requires at least 3 samples for stable novelty scoring.
        if len(Xs) >= 3:
            n_neighbors = min(20, len(Xs) - 1)
            ...
            self.lof = None
            logger.warning("Skipping LOF fit due to small sample size (<3)")
    ...
    def predict(self, df):
        ...
        lof_scores = self.lof.decision_function(Xs) if self.lof is not None else np.zeros(len(Xs))
